...
class ToDoList extends React.Component {
    ...
    renderToDoListItem(itemIdentifier) {
      ...
      const todo = this.props.todos.filter(todo => todo["id"] === itemIdentifier)[0];
      return <ToDoListItem key={todo["id"]} id={todo["id"]} toDoName={todo["toDoName"]} toDoStatus={todo["toDoStatus"]} onStatusChange={this.handleStatusChange}></ToDoListItem>
    }
    ...
    render() {
      ...
      if(filter !== "all") {
        ...
        const todo_array = todos.filter(todo => todo["toDoStatus"] === filter).map(todo => {
